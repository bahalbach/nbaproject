...
from nba_dataclasses import expected_offense_team
...
def get_num_fta_from_foul(foul: enhanced_pbp.Foul):
    ...
    while event and event.clock == clock:
        ...
        if event.event_type == 6 and not (event.is_technical or event.is_double_technical or event.is_flagrant):
            ...
            if number_of_fta_for_foul == 0:
                nonflagrant_fts = get_num_nonflagrant_fts(event)
            break
            # print("flagrant fts", flagrant_fts)
        # 3 = free throw
        elif event.event_type == 3:
            if event.is_technical_ft or event.is_flagrant_ft:
                pass
            elif event.foul_that_led_to_ft is foul:
                seen_ft += 1
                if event.num_ft_for_trip is None:
                    print("no fts?", event)
        ...
        pass
    # print("current event is", event, number_of_fta_for_foul, nonflagrant_fts)

    if max(number_of_fta_for_foul, nonflagrant_fts) == 0:
    ...
    return max(number_of_fta_for_foul, nonflagrant_fts)
...
def get_num_nonflagrant_fts(foul):
    clock = foul.clock
    event = foul.next_event
    number_of_fta_for_foul = 0
    while event and event.clock == clock:
        if event.event_type == 3 and not event.is_flagrant_ft and not event.is_technical_ft:
            number_of_fta_for_foul = event.num_ft_for_trip
        event = event.next_event
    return number_of_fta_for_foul
...
missing_ge_1.result = PossessionTry(TryResultType.OTHER)
...
def process_season(season, first_game=0, check_fts=False):
    ...
    for game_dict in season.schedule.games.final_games[first_game:]:
        ...
        game_events = game_possession_info.game_events
        if check_fts:
            num_fts = 0
            expected_fts = 0
            ft_violations = 0
            ...
            for i in range(len(game_events)):
                e = game_events[i]
                if hasattr(e.result, "num_fts") and e.result.num_fts:
                    expected_fts += e.result.num_fts
                    if e.num_simultanious_flagrant_fts:
                        # print("has sffts", e.num_simultanious_flagrant_fts)
                        # print(e)
                        expected_fts += e.num_simultanious_flagrant_fts
                        # should_break = True

                if e.event_type == EventType.LiveFreeThrow:
                    if e.result.result_type in {LiveFreeThrowResult.DEF_LANE_VIOLATION_RETRY, }:
                        ft_violations += 1
                    else:
                        num_fts += 1
            num_fts += len(game_possession_info.free_throws)
            # for n in (game_possession_info.technicals[0] | game_possession_info.technicals[1]).values():
            #     tech_num += n
            # tech_num += game_possession_info.technicals[2]
            tech_num = game_possession_info.team_techs[0] + \
                game_possession_info.team_techs[1]
            if game_possession_info.team_techs[2] != 0:
                print("other techs?",
                      game_possession_info.team_techs[2], game_id)

            if num_fts != expected_fts+tech_num:
                print("for game", game_id)
                print("not equal", num_fts, ft_violations,
    ...
    for possession in possessions:
        ...
        for event in possession.events:
            ...
            if isinstance(event, enhanced_pbp.FieldGoal):
                ...
                try_result.is_made = event.is_made
                ...
                if is_reboundable(game_events[-1]) or (try_start.start_type is None and missing_start is None and possible_missing_start is None):
                    ...
                    if isinstance(next_event, enhanced_pbp.Rebound) and next_event.oreb:
                        ...
                        rebound = Rebound(shooter, shot_type, is_blocked,
                                          rebound_result, rebounder)
                        out_of_order_rebound_game_event.result = rebound
                ...
                if expected_fts == 0 and not event.is_technical_ft:
                    if possession_after:
                        possession_after = False
                        logging.debug(
                            f"possession after so not reboundable")
                    else:
                        event.is_reboundable = True
                        logging.debug(
                            f"reboundable")
                        ...
                        ft_result = LiveFreeThrow(
                            None, event.player1_id, goaltender)
                        live_free_throw.result = ft_result

                        if event.is_made:
                            if goaltended:
                                ft_result.result_type = LiveFreeThrowResult.DEF_GOALTEND_MAKE
                            else:
                                ft_result.result_type = LiveFreeThrowResult.MADE
                        else:
                            ft_result.result_type = LiveFreeThrowResult.MISS
                    ...
                    logging.debug(
                        f"more fts left so not reboundable")

                if not event.is_reboundable:
                    ft = FreeThrow(event.player1_id, event.is_made,
                                   expected_fts, possession_after, score_margin, event.is_technical_ft)
                ...
                # TODO handle for rebound fouls/made shot/ft fouls (flagrant w/ 2 fts?)
                fouler = event.player1_id
                ...
                if start_of_game_or_overtime:
                    ...
                    jumpball = JumpBall(winning_team)

                    if number_of_fta_for_foul != 0:
                        jumpball.result_type = JumpballResultType.LOOSE_BALL_FOUL
                        jumpball.num_fts = number_of_fta_for_foul
                    ...
                    jumpball_game_event.result = jumpball
                    game_events.append(jumpball_game_event)
                    start_of_game_or_overtime = False
                    logging.debug(
                        f"process foul {event.event_num} as start of game foul")
                    continue
                ...
                if isinstance(event.previous_event, enhanced_pbp.JumpBall) and event.clock == event.previous_event.clock:
                    ...
                    if number_of_fta_for_foul != 0:
                        game_events[-1].result.result_type = JumpballResultType.LOOSE_BALL_FOUL
                        game_events[-1].result.num_fts = number_of_fta_for_foul

                    if event.is_clear_path_foul:
                        game_events[-1].result.result_type = JumpballResultType.CLEAR_PATH_FOUL
                    ...
                    logging.debug(
                        f"process foul {event.event_num} as jumpball foul")
                    continue

                if not and1 and not (event.is_double_technical or event.is_technical or event.is_away_from_play_foul) and number_of_fta_for_foul == 1:
                    next_event = event.next_event
                    while isinstance(next_event, (enhanced_pbp.FreeThrow, enhanced_pbp.Replay, enhanced_pbp.Timeout, enhanced_pbp.Substitution, enhanced_pbp.Violation)):
                        next_event = next_event.next_event
                    if next_event and next_event.clock == event.clock and isinstance(next_event, enhanced_pbp.FieldGoal) and next_event.is_made and next_event.team_id != event.team_id:
                        # if not next_event.is_and1:
                        #     print("not and1, and1", event, next_event)
                        #     print()
                        out_of_order_and1_foul = event
                        logging.debug(
                            f"process foul {event.event_num} as out of order and1")
                        continue

                if and1 and not (event.is_double_technical or event.is_technical):
                    # add fouler to and1 TryResult and don't add this
                    if game_events[-1].result.result_type != TryResultType.AND1 and game_events[-1].result.result_type != TryResultType.GOALTENDED_AND1:
                        print("error")
                        print(event, possession.events)
                        raise Exception(possession, game_events)

                    game_events[-1].result.result_player3_id = fouler
                    game_events[-1].result.num_fts = number_of_fta_for_foul
                    if game_events[-1].result.num_fts != 1 and not event.is_flagrant:
                    ...
                    if event.is_flagrant:
                        ...
                        if event.is_flagrant1:
                            if number_of_fta_for_foul == 1:
                                game_events[-1].result.result_type = TryResultType.FLAGRANT1_AND1
                            else:
                                game_events[-1].result.result_type = TryResultType.FLAGRANT1_AND1_2FTS
                        elif event.is_flagrant2:
                            game_events[-1].result.result_type = TryResultType.FLAGRANT2_AND1
                        try_start = TryStart(
                            TryStartType.AFTER_FOUL, period_time_left)
                    ...
                    if event.is_away_from_play_foul or event.is_loose_ball_foul or event.is_personal_foul or event.is_personal_block_foul:
                        ...
                        while next_event and next_event.clock == event.clock:
                            if isinstance(next_event, enhanced_pbp.Turnover) and next_event.is_no_turnover:
                                ...
                                break
                            next_event = next_event.next_event
                    logging.debug(f"process foul {event.event_num} as and1")
                    continue

                made_shot_with_foul = ((game_events[-1].event_type == EventType.PossessionTry)
                                       and game_events[-1].result.result_type == TryResultType.MADE_SHOT)
                made_ft_with_foul = ((game_events[-1].event_type == EventType.LiveFreeThrow)
                                     and game_events[-1].result.result_type == LiveFreeThrowResult.MADE)
                wrong_foul_type = event.is_offensive_foul or event.is_charge or event.is_inbound_foul or event.is_double_foul or event.is_personal_take_foul or event.is_clear_path_foul
                has_turnover_at_same_time = False
                next_event = event.next_event
                while next_event and next_event.clock == event.clock:
                    if isinstance(next_event, enhanced_pbp.Turnover) and not next_event.is_no_turnover:
                        has_turnover_at_same_time = True
                        break
                    next_event = next_event.next_event
                    ...
                    if game_events[-1].lineup.offense_team == event.team_id or wrong_foul_type or has_turnover_at_same_time:
                        ...
                        if made_shot_with_foul:
                            if event.is_flagrant:
                                possession_after = True
                                if event.is_flagrant1:
                                    game_events[-1].result.result_type = TryResultType.MADE_BASKET_W_FLAGRANT1
                                else:
                                    game_events[-1].result.result_type = TryResultType.MADE_BASKET_W_FLAGRANT2
                            else:
                                game_events[-1].result.result_type = TryResultType.MADE_BASKET_W_FOULSHOT
                            game_events[-1].result.result_player3_id = fouler
                            game_events[-1].result.result_player4_id = fouled
                            game_events[-1].result.num_fts = number_of_fta_for_foul
                            ...
                            if event.is_flagrant:
                                possession_after = True
                                if event.is_flagrant1:
                                    game_events[-1].result.result_type = LiveFreeThrowResult.MADE_AND_FLAGRANT1
                                else:
                                    game_events[-1].result.result_type = LiveFreeThrowResult.MADE_AND_FLAGRANT2
                            else:
                                game_events[-1].result.result_type = LiveFreeThrowResult.MADE_AND_FOUL
                            game_events[-1].result.fouler = fouler
                            game_events[-1].result.fouled = fouled
                            game_events[-1].result.num_fts = number_of_fta_for_foul
                            if game_events[-1].lineup.offense_team == event.team_id:
                                print("made ft w offensive foul",
                                      game_events[-1].result.result_type)
                ...
                if double_lane_violation:
                    ...
                    # check if 2 or 3 free throws
                        ...
                        if isinstance(event.next_event, enhanced_pbp.FieldGoal) and event.next_event.is_made and next_event.clock == event.clock:
                            out_of_order_and1_foul = event
                            logging.debug(
                                f"process foul {event.event_num} as out of order and1 shooting foul")
                            continue
                        else:
                    ...
                    team_techs[event.team_id] -= 1  # don't count as a tech
                    has_def_try_result = True
                    next_try_start.start_type = TryStartType.AFTER_FOUL
                ...
                if event.is_goaltend_violation:
                    ...
                    if next_event.clock == event.clock and (isinstance(next_event, enhanced_pbp.FreeThrow)):
                        ...
                        if event.team_id == ft_team:
                            ft_result.result_type = LiveFreeThrowResult.OFF_LANE_VIOLATION_RETRY
                        else:
                            ft_result.result_type = LiveFreeThrowResult.DEF_LANE_VIOLATION_RETRY
                        ft_result.fouler = event.player1_id
                        ft_result.fouled = None
                    else:
                        double_lane_violation = True
                        ...
                        ft_result.result_type = LiveFreeThrowResult.DOUBLE_LANE_VIOLATION
                        ft_result.fouler = violater1
                        ft_result.fouled = violater2
                        expected_fts -= 1
                    ...
                    if expected_fts - expected_tfts > 0:
                        if expected_fts != 1:
                            ...
                            if event.team_id == offense_team_id:
                                print("lane violation w exp fts",
                                      expected_fts, event)
                                print(possession.events)
                                # TODO add as OFF_LANE_VIOLATION_MISS
                                expected_fts -= 1
                                # raise Exception(possession, game_events)
                                pass
                            else:
                        ...
                        if event.team_id != ft_team or next_event.clock == event.clock and (isinstance(next_event, enhanced_pbp.FreeThrow) or (isinstance(next_event, enhanced_pbp.Violation) and (next_event.is_lane_violation or next_event.is_double_lane_violation)) or (event.team_id != ft_team and isinstance(next_event, enhanced_pbp.Turnover) and next_event.is_lane_violation)):
                            ...
                            # if event.team_id == offense_team_id:
                            ft_result.result_type = LiveFreeThrowResult.OFF_LANE_VIOLATION_MISS
                            # else:
                            #     live_free_throw.result_type = LiveFreeThrowResult.DEF_LANE_VIOLATION_MAKE
                    ...
                    if expected_tfts == 0:
                        ...
                        if isinstance(rebound, enhanced_pbp.Violation) and rebound.team_id == event.team_id:
                            ...
                            if event.team_id != missed_ft.team_id:
                                ...
                                game_events[-2].result.result_type = LiveFreeThrowResult.OFF_LANE_VIOLATION_MISS
                                game_events.pop()
                            ...
                            if event.team_id == rebound.team_id:
                                ...
                                game_events[-1].result.result_type = LiveFreeThrowResult.DEF_LANE_VIOLATION_MAKE
                        else:
                        ...
                        print("lane violation with just techs",
                              event, "not sure what this means, ignoring")
                ...
                logging.debug(
                    f"turnover {event.event_num}: {event.event_action_type}")
                if offensive_foul_turnover:
                    # turnover already handled in the foul event
                    offensive_foul_turnover = False
                    logging.debug(f"ignoring off foul turnover")

                elif loose_ball_foul_turnover:
                    # turnover already handled in the foul event
                    loose_ball_foul_turnover = False
                    logging.debug(f"ignoring lb foul turnover")

                elif handled_jumpball_turnover:
                    handled_jumpball_turnover = False
                    logging.debug(f"ignoring jumpball turnover")
                ...
                # Illegal screen, 43 is out of bounds screen
                    ...
                    if last_event.event_type == EventType.Rebound and last_event.period_time_left == period_time_left and game_events[-2].lineup.offense_team == event.team_id:
                        game_events[-1].result.result_type = ReboundResult.KICKED_BALL_TURNOVER
                        try_start = TryStart(
                            TryStartType.DEAD_BALL_TURNOVER, period_time_left)
                    ...
                    if last_event.event_type == EventType.Rebound and last_event.period_time_left == period_time_left:
                        if game_events[-2].lineup.offense_team == event.team_id:
                            game_events[-1].result.result_type = ReboundResult.SHOTCLOCK_TURNOVER
                            try_start = TryStart(
                                TryStartType.DEAD_BALL_TURNOVER, period_time_left)
                        else:
                    ...
                    if is_free_throw:
                        ...
                        shooter = find_free_throw_shooter(event)
                        goaltender = event.player1_id
                        ft_result = LiveFreeThrow(
                            LiveFreeThrowResult.OFF_GOALTEND_TURNOVER, shooter, goaltender, None, None)
                        live_free_throw.result = ft_result
                        expected_fts -= 1
                        game_events.append(live_free_throw)
                        continue
                        ...
                        if isinstance(event.previous_event, enhanced_pbp.Rebound) and event.previous_event.is_placeholder:
                            if game_events[-1].event_type != EventType.Rebound:
                                ...
                                raise Exception(possession, game_events)
                            game_events[-1].result.result_type = ReboundResult.OFF_REBOUND
                            game_events[-1].result.rebounder = event.player1_id
                    ...
                    if isinstance(prev_event, enhanced_pbp.Rebound) and event.clock == prev_event.clock:
                        ...
                        if game_events[-1].event_type == EventType.Rebound:
                            game_events[-1].result.result_type = rebound_result
                            game_events[-1].result.fouler = fouler
                        else:
                    ...
                    if expected_fts - expected_tfts != 0:
                        ...
                        fouler = event.player1_id
                        ft_result = LiveFreeThrow(
                            LiveFreeThrowResult.OFF_LANE_VIOLATION_MISS, shooter, None, fouler, None)
                        live_free_throw.result = ft_result
                        ...
                        if event.previous_event.clock == event.clock:
                            last_event_missed_live_ft = game_events[-2].event_type == EventType.LiveFreeThrow and game_events[
                                -2].result.result_type == LiveFreeThrowResult.MISS and game_events[-2].period_time_left == period_time_left
                            if last_event_missed_live_ft:
                                game_events.pop()
                                game_events[-1].result.result_type = LiveFreeThrowResult.OFF_LANE_VIOLATION_MISS
                                game_events[-1].result.fouler = event.player1_id
                                continue
                            else:
                ...
                if reboundable:
                    ...
                    if loose_ball_foul_before_rebound:
                        logging.debug(
                            f"loose ball foul before rebound {event.event_num}")
                        ...
                        if isinstance(next_event, enhanced_pbp.Foul) and next_event.is_loose_ball_foul and isinstance(next_event.next_event, enhanced_pbp.Turnover) and (next_event.next_event.is_no_turnover or next_event.next_event.event_action_type == 5) and time_diff < 2:
                            ...
                            while next_event and next_event.clock == event.clock:
                                if isinstance(next_event, enhanced_pbp.Foul) and next_event.is_loose_ball_foul:
                                    loose_ball_foul_rebound = True
                                    break
                                elif isinstance(next_event, (enhanced_pbp.JumpBall, enhanced_pbp.Foul, enhanced_pbp.Rebound, enhanced_pbp.Violation)) or (isinstance(next_event, enhanced_pbp.FieldGoal) and next_event.is_made) or (isinstance(next_event, enhanced_pbp.Turnover) and not(next_event.is_no_turnover or next_event.event_action_type == 5)):
                                    break
                                next_event = next_event.next_event
                        if loose_ball_foul_rebound:
                    ...
                    if loose_ball_foul_rebound:
                        ...
                        logging.debug(
                            f"loose ball foul rebound {event.event_num}, w fts {num_fts}")
                ...
                if event.support_ruling or event.ruling_stands:
                    ...
                    if isinstance(event.next_event, enhanced_pbp.JumpBall):
                        if game_events[-1].event_type == EventType.PossessionTry and game_events[-1].result.result_type == TryResultType.MISTAKE_CALL:
                            pass
                        else:
            ...
            if process_jumpball:
                ...
                if start_of_game_or_overtime:
                    ...
                    while next_event and (is_jumpball_violation(next_event) or is_jumpball_turnover(next_event)):
                        next_event = next_event.next_event
                    if isinstance(next_event, enhanced_pbp.Turnover) and next_event.clock == event.clock:
                    ...
                    # last try ends with a held ball
                        ...
                        if is_reboundable(game_events[-1]) and not hasattr(last_shot_or_ft, "has_been_rebounded"):
                            ...
                            original_offensive_team = expected_offense_team(
                                game_events)
                            if original_offensive_team is None:
                                next_event = event.next_event
                                while next_event and next_event.clock == event.clock:
                                    if isinstance(next_event, enhanced_pbp.Turnover):
                                        if winning_team == defense_team_id:
                                            original_offensive_team = offense_team_id
                                        else:
                                            original_offensive_team = defense_team_id
                                        break
                                    next_event = next_event.next_event
                                else:
                                    if winning_team == offense_team_id:
                                        original_offensive_team = offense_team_id
                                    else:
                                        original_offensive_team = defense_team_id
                            has_held_ball_result = True
            ...
            if has_off_try_result or has_def_try_result or has_held_ball_result:
                logging.debug(
                    f"process try result {event.event_num}: {try_result.result_type}")
                if start_of_game_or_overtime:
                    ...
                    jumpball = JumpBall(winning_team=offense_team_id)
                    jumpball_game_event.result = jumpball
                ...
                if (has_off_try_result and event.team_id != offense_team_id) or (has_def_try_result and event.team_id == offense_team_id):
                    ...
                    in_penalty = fouls_to_give == 0
                    score_margin = -score_margin
                elif has_held_ball_result:
                    lineup = get_foul_lineup(
                        prev_event, original_offensive_team)
                    offense_is_home = original_offensive_team == home_team
                    fouls_to_give = prev_event.fouls_to_give[
                        road_team if offense_is_home else home_team]
                    in_penalty = fouls_to_give == 0
                    if original_offensive_team != offense_team_id:
                        score_margin = -score_margin
                else:
                    lineup = get_foul_lineup(event, offense_team_id)
                    offense_is_home = offense_team_id == home_team
                ...
                try_result.try_start = try_start
                try_game_event.result = try_result
                ...
                if hasattr(last_reboundable_shot, "has_been_rebounded"):
                    ...
                    raise Exception(possession, game_events)
                logging.debug(
                    f"process rebound {event.event_num}: {rebound_result_type}")
                ...
                rebound = Rebound(
                    shooter, shot_type, is_blocked, rebound_result_type, rebounder, fouler, num_fts)
                rebound_game_event.result = rebound
